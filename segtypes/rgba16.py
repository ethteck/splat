...
class N64SegRgba16(N64Segment):
    ...
    def iter_image_indexes(self, bytes_per_x = 1, bytes_per_y = 1):
        ...
        xrange = range(w - ceil(bytes_per_x), -1, -ceil(bytes_per_x)) if self.flip_horizontal else range(0, w, ceil(bytes_per_x))
        yrange = range(h - ceil(bytes_per_y), -1, -ceil(bytes_per_y)) if self.flip_vertical else range(0, h, ceil(bytes_per_y))
    ...
    def split(self, rom_bytes, base_path):
        if self.type in self.options["modes"] or "img" in self.options["modes"] or "all" in self.options["modes"]:
