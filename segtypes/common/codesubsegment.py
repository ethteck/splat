...
class CommonSegCodeSubsegment(Segment):
    ...
    def scan_code(self, rom_bytes, is_asm=False):
        ...
        for jtbl_label_vram in self.parent.jtbl_glabels_to_add:
            ...
            sym = self.get_most_parent().create_symbol(jtbl_label_vram, True, type="jtbl_label", define=True)
            sym.rom = rom_addr
    ...
    def process_insns(
        ...
        func_sym = self.get_most_parent().create_symbol(func_spim.vram, True, type="func", define=True)
        func_spim.setNameGetCallback(lambda _: func_sym.name)
        if func_sym.rom is None:
            func_sym.rom = func_spim.vromStart
        ...
        for referenced_vram in func_spim.referencedVRams:
            ...
            if context_sym is not None:
                in_segment = context_sym.overlayCategory == self.get_most_parent().get_exclusive_ram_id()
        ...
        for label_offset in func_spim.localLabels:
            ...
            label_sym = self.get_most_parent().create_symbol(
                label_vram, True, reference=True, local_only=True
            )
            if not label_sym.type:
                label_sym.type = "label"
            ...
            if label_sym is not None:
                ...
                if context_sym is not None:
                    ...
                    if context_sym.nameGetCallback is None:
                        context_sym.setNameGetCallback((lambda x: (lambda _: x.name))(label_sym))
