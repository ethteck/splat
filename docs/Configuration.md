...
Emit an `-include` directive in the generated linker dependency file that includes `.o` file dependencies, if they exist.  

An `-include` directive is appended to the generated dependency file (see [`ld_dependencies`](#ld_dependencies)), listing each `.o` dependency of the `.elf` target as its corresponding `.d` file (e.g. `bar.o` → `bar.d`).

These dependency files need to be generated by your build system; refer to GCC's `-M` family of flags for details.
