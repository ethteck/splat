...
"""
    ...
    if (
        ...
        bss_start = rom.entrypoint_info.bss_start_address.value - rom.entry_point + 0x1000
...
"""
    ...
    if rom.entrypoint_info.bss_start_address is not None:
        reloc_addrs.append(f"rom:0x{rom.entrypoint_info.bss_start_address.rom_hi:06X} reloc:MIPS_HI16 symbol:main_BSS_START")
        reloc_addrs.append(f"rom:0x{rom.entrypoint_info.bss_start_address.rom_lo:06X} reloc:MIPS_LO16 symbol:main_BSS_START")
        reloc_addrs.append("")
    if rom.entrypoint_info.bss_size is not None:
        reloc_addrs.append(f"rom:0x{rom.entrypoint_info.bss_size.rom_hi:06X} reloc:MIPS_HI16 symbol:main_BSS_SIZE")
        reloc_addrs.append(f"rom:0x{rom.entrypoint_info.bss_size.rom_lo:06X} reloc:MIPS_LO16 symbol:main_BSS_SIZE")
        reloc_addrs.append("")
    if rom.entrypoint_info.bss_end_address is not None:
        reloc_addrs.append(f"rom:0x{rom.entrypoint_info.bss_end_address.rom_hi:06X} reloc:MIPS_HI16 symbol:main_BSS_END")
        reloc_addrs.append(f"rom:0x{rom.entrypoint_info.bss_end_address.rom_lo:06X} reloc:MIPS_LO16 symbol:main_BSS_END")
        reloc_addrs.append("")
    if rom.entrypoint_info.stack_top is not None:
        reloc_addrs.append('// This entry corresponds to the "stack top", which is the end of the array used as the stack for the main segment.')
        reloc_addrs.append('// It is commented out because it was not possible to infer what the start of the stack symbol is, so you\'ll have to figure it out by yourself.')
        reloc_addrs.append('// Once you have found it you can properly name it and specify the length of this stack as the addend value here.')
        reloc_addrs.append(f'// The address of the end of the stack is 0x{rom.entrypoint_info.stack_top.value:08X}.')
        reloc_addrs.append(f'// A common size for this stack is 0x2000, so try checking for the address 0x{rom.entrypoint_info.stack_top.value-0x2000:08X}. Note the stack may have a different size.')
        reloc_addrs.append(f"// rom:0x{rom.entrypoint_info.stack_top.rom_hi:06X} reloc:MIPS_HI16 symbol:main_stack addend:0xXXXX")
        reloc_addrs.append(f"// rom:0x{rom.entrypoint_info.stack_top.rom_lo:06X} reloc:MIPS_LO16 symbol:main_stack addend:0xXXXX")
        reloc_addrs.append("")
    if reloc_addrs:
        with Path("reloc_addrs.txt").open("w", newline="\n") as f:
          print("Writing reloc_addrs.txt")
          f.write("// Visit https://github.com/ethteck/splat/wiki/Advanced-Reloc for documentation about this file")
    ...
    if rom.entrypoint_info.main_address is not None:
        symbol_addrs.append(f"main = 0x{rom.entrypoint_info.main_address.value:08X}; // type:func")
    if symbol_addrs:
        symbol_addrs.append("")
        with Path("symbol_addrs.txt").open("w", newline="\n") as f:
          print("Writing symbol_addrs.txt")
          f.write("// Visit https://github.com/ethteck/splat/wiki/Adding-Symbols for documentation about this file")
          contents = "\n".join(symbol_addrs)
          f.write(contents)
