...
* BREAKING CHANGES:
  ...
  * It is pretty likely for many symbols that have autogenerated names to change their naming a bit.
    * In the past splat failed to properly set some attributes required for autogenerated symbol naming, so splat symbolized them "wrong". For example it only used the vram address (like `D_{VRAM}`) when it should have been using both the vram and rom addresses (`D_{VRAM}_{ROM}`) for naming overlay symbols.
    * Now those attributes are correctly set, meaning those symbols are correctly named now.
    * This is breaking if you were referencing those wrongly named symbols in C code, because you'll get linker errors for not finding the old named symbols.
  * Versions between 0.41.0 and 0.50.0 have been skipped as a way to note this can be a pretty big breaking change.
